...
`client.Disconnect()`, the active transport's `Disconnected` event, and connecting a different transport all end the current client session. They detach every known entity, clear ownership and all profiles (including `client.Profile`), and discard pending entity operations, RPCs, broadcasts, and profile updates. Detached entity instances retain their ids and data, but report `IsSpawned = false` and `IsOwner = false`; call `Spawn` explicitly to use an instance in a later session. A new connection does not resume or reconcile the previous server's state.

Teardown clears the entire session before raising `EntityOwnershipLost` for previously owned entities, `EntityUnobserved` for all known entities, and `ProfileLeft` for all known profiles. These notifications describe the ended session. Callbacks may prepare a new session; a connection or disconnection requested by a teardown callback takes precedence over the outer `Connect` call. An interrupted `Tick` stops processing the ended session, and recursive calls to `Tick` are ignored until the active tick returns. If a broadcast's local handler changes the session or assigned profile, the broadcast throws `InvalidOperationException` before serializing or queueing its outgoing message; the local handler has already run.

Entities may be prepared with `Spawn` while disconnected, including before the first connection; `Connect` retains that prepared state when there is no active transport. Calling `Disconnect` also clears prepared state. Connecting the same active transport is a no-op, and a null transport is rejected without changing the current session. The client does not close or dispose caller-owned transports: the caller must close the old transport to notify the server and release its resources. Generated message writers belong to the session in which they were rented and cannot be queued after that session ends.
