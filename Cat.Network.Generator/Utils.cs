...
namespace Cat.Network.Generator {
	internal static class Utils {
		...
		private static Dictionary<string, StringTemplate> SerializationTemplates { get; } = new Dictionary<string, StringTemplate>() {
			...
			{ "System.Collections.Immutable.ImmutableArray<System.Int16>",  new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteInt16LittleEndian({PropertyBufferName},  __element); {PropertyBufferName} = {PropertyBufferName}.Slice(2); }} }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Int32>",  new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName},  __element); {PropertyBufferName} = {PropertyBufferName}.Slice(4); }} }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Int64>",  new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteInt64LittleEndian({PropertyBufferName},  __element); {PropertyBufferName} = {PropertyBufferName}.Slice(8); }} }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.UInt16>", new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteUInt16LittleEndian({PropertyBufferName}, __element); {PropertyBufferName} = {PropertyBufferName}.Slice(2); }} }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.UInt32>", new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteUInt32LittleEndian({PropertyBufferName}, __element); {PropertyBufferName} = {PropertyBufferName}.Slice(4); }} }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.UInt64>", new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteUInt64LittleEndian({PropertyBufferName}, __element); {PropertyBufferName} = {PropertyBufferName}.Slice(8); }} }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Single>", new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteSingleLittleEndian({PropertyBufferName}, __element); {PropertyBufferName} = {PropertyBufferName}.Slice(4); }} }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Double>", new StringTemplate($"{PropertyBufferName}[0] = ({{name}}.IsDefault ? (byte) 0 : (byte) 1); {PropertyBufferName} = {PropertyBufferName}.Slice(1); if (!{{name}}.IsDefault) {{ {BinaryPrimitivesFQN}.WriteInt32LittleEndian({PropertyBufferName}, {{name}}.Length); {PropertyBufferName} = {PropertyBufferName}.Slice(4); foreach (var __element in {{name}}) {{ {BinaryPrimitivesFQN}.WriteDoubleLittleEndian({PropertyBufferName}, __element); {PropertyBufferName} = {PropertyBufferName}.Slice(8); }} }}", "name") },
		};
		...
		private static Dictionary<string, StringTemplate> DeserializationTemplates { get; } = new() {
			...
			{ "System.Collections.Immutable.ImmutableArray<System.Int16>",  new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.Int16[]  newValue = new System.Int16[immutableArrayLength];  for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadInt16LittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(2); }} {{name}} =  System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Int32>",  new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.Int32[]  newValue = new System.Int32[immutableArrayLength];  for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(4); }} {{name}} =  System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Int64>",  new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.Int64[]  newValue = new System.Int64[immutableArrayLength];  for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadInt64LittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(8); }} {{name}} =  System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.UInt16>", new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.UInt16[] newValue = new System.UInt16[immutableArrayLength]; for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadUInt16LittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(2); }} {{name}} = System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.UInt32>", new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.UInt32[] newValue = new System.UInt32[immutableArrayLength]; for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadUInt32LittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(4); }} {{name}} = System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.UInt64>", new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.UInt64[] newValue = new System.UInt64[immutableArrayLength]; for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadUInt64LittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(8); }} {{name}} = System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Single>", new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.Single[] newValue = new System.Single[immutableArrayLength]; for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadSingleLittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(4); }} {{name}} = System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") },
			{ "System.Collections.Immutable.ImmutableArray<System.Double>", new StringTemplate($"if ({PropertyBufferName}[0] == 1) {{ {PropertyBufferName} = {PropertyBufferName}.Slice(1); System.Int32 immutableArrayLength = {BinaryPrimitivesFQN}.ReadInt32LittleEndian({PropertyBufferName}); System.Double[] newValue = new System.Double[immutableArrayLength]; for (int immutableArrayIndex = 0; immutableArrayIndex < immutableArrayLength; immutableArrayIndex++) {{ newValue[immutableArrayIndex] = {BinaryPrimitivesFQN}.ReadDoubleLittleEndian({PropertyBufferName}); {PropertyBufferName} = {PropertyBufferName}.Slice(8); }} {{name}} = System.Runtime.InteropServices.ImmutableCollectionsMarshal.AsImmutableArray(newValue); }} else {{ {{name}} = default; }}", "name") }
		};
